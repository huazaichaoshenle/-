...
- (void)initCompressionSession{
    if(compressionSession){
        ...
        //当你完成了一个压缩会话创建,VTCompressionSessionInvalidate打电话把它拆下来，然后CFRelease释放你的对象引用。
        VTCompressionSessionInvalidate(compressionSession);
        CFRelease(compressionSession);
        compressionSession = NULL;
    }
    //VTCompressionSessionCreate:为压缩视频帧创建一个会话。
...
static void VideoCompressonOutputCallback(void *VTref, void *VTFrameRef, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer)
{
    ...
    {
        //CMFormatDescriptionRef:对CMFormatDescription的引用，一个用于描述特定类型的媒体的CF对象(音频、视频、muxed等);  CMFormatDescription:图像存储方式，编解码器等格式描述
    ...
    char *dataPointer;
    //CMBlockBufferGetDataPointer:获得对由CMBlockBuffer所表示的数据的访问。
